...
## `btc-rpc-explorer-patoshi-history.json` — the same list, at each upstream commit

Six snapshots of the `Patoshi.heights` array as it stood at each commit of the upstream file, with the commit hash, date, message, count and the SHA-256 of the height set. Generated by `verify/patoshi_list_history.py` from the same repository, <https://github.com/janoside/btc-rpc-explorer>, MIT licensed; copyright remains with its authors. It exists so that "the list changed" is a checkable statement rather than a recollection.
