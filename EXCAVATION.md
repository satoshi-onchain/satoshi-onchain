...
- **Distinct coinbase pubkeys per block** (P2PK output scripts) — to confirm fresh-key-per-block and
  count distinct Satoshi keys. Needs the coinbase `scriptPubKey` (not in `early_blocks.csv`).
- **Implied hashrate / inter-block gap** vs difficulty — the throttling (Patoshi held ~50% not ~100%).
- **Full nonce high-word structure** and per-thread nonce partition (the multi-thread reconstruction).
